...
function updatepreferencesdynamic()
{
	...
	title = fb.playlistPage != 1 ? 
					'第 '+(fb.playlistPage - 1)+' 页/共 '+Math.ceil(fb.playlistItemsCount / fb.playlistItemsPerPage)+' 页' : ''
...
function updateplaylist()
{
	...
	if (fb.isPlaying == "1")
		tmp += ['正在播放 ', playingItem, ' / ', fb.playlistPlayingItemsCount, ' | ', fb.helper3].join('');
	else
	if (fb.isPaused == "1")
		tmp += ['已暂停 ', playingItem, ' / ', fb.playlistPlayingItemsCount, ' | ', fb.helper3].join('');
	else
		tmp += '已停止 ';
...
function updateui()
{
	if (fb)
	{
		...
		if (fb.volumedb == 1000)
			$('#mute').addClass('ui-state-error').attr('title', '解除静音');
		else
			$('#mute').removeClass('ui-state-error').attr('title', '静音');
	...
	$(document).ready(function() {
		...
		$('#pageslider').slider({
		    ...
		    slide: function(event,ui) {	
		        tooltip.show( '第 ' + ui.value + ' 页/共 ' + (Math.ceil(fb.playlistItemsCount*1.0 / fb.playlistItemsPerPage)+ ' 页') , mouse.x+20 , ($(this).offset().top - 15) );
		    },
			range: false,
			min: 1,
			max: 1,            
		...
		$('#browse_dlg').dialog({
			...
			buttons: {
				"浏览到..." : function() { 
					var path = promptf("浏览到:", Url.decode(br.pathcurrent));
		...
		$('#CreatePlaylist').click(function(e) {
			var name = promptf('输入新建播放列表名称:', '新建播放列表');
		...
		$('#RemovePlaylist').click(function(e) {
			if (!$(this).hasClass('ui-state-disabled'))
				if (confirm("确认移除 \""+fb.playlists[fb.playlistActive].name+"\" 的播放列表吗?"))
		...
		$('#RenamePlaylist').click(function(e) {
			if (!$(this).hasClass('ui-state-disabled'))
			{
				var new_name = promptf("输入新的名称:", fb.playlists[fb.playlistActive].name);
